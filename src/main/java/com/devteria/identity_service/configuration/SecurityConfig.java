...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity httpSecurity) throws Exception {
        ...
        httpSecurity.authorizeHttpRequests(
                requests -> requests.requestMatchers(HttpMethod.POST, PUBLIC_ENDPOINTS)
                        .permitAll()
                        //                        .requestMatchers(HttpMethod.GET, "/users").hasAuthority("SCOPE_ADMIN")
                        //                        .requestMatchers(HttpMethod.GET, "/users").hasAuthority("ROLE_ADMIN")
                        //                        .requestMatchers(HttpMethod.GET, "/users").hasRole(Role.ADMIN.name())
                        .anyRequest()
                        .authenticated() // Tất cả request phải đc authenticated mới đc access
                );
        ...
        httpSecurity.oauth2ResourceServer(
                oauth2 -> oauth2.jwt(jwtConfigurer -> jwtConfigurer
                                ...
                                .authenticationEntryPoint(new JwtAuthenticationEntryPoint()) // Khi có lỗi xác thực (ví dụ token sai, hết hạn),
                                                                                                // Spring sẽ gọi JwtAuthenticationEntryPoint để xử lý.
                );
